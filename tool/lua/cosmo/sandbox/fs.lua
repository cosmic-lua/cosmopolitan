...
--
-- All helpers return `nil, err` on failure, where `err` is either a
-- `unix.Errno` (for direct syscall failures) or a string (for
-- composite errors that wrap a pre-condition check). Callers that
-- want the raw errno can `:errno()` the Errno object.
--
-- Linux-only.
...
local function ensure_mountpoint(src, dst)
  ...
  if not st then return nil, "missing source: " .. src end
...
function M.bind(src, dst, opts)
  ...
  if not ok then return nil, "bind " .. src .. ": " .. tostring(err) end
  if opts.ro then
    flags = unix.MS_REMOUNT | unix.MS_BIND | unix.MS_RDONLY
            | (rec and unix.MS_REC or 0)
    ok, err = unix.mount("none", dst, nil, flags, nil)
    if not ok then
      return nil, "ro-remount " .. dst .. ": " .. tostring(err)
    end
  end
  return true
end
...
function M.tmpfs(dst, size_or_data)
  ...
  if not ok then return nil, "tmpfs " .. dst .. ": " .. tostring(err) end
  return true
end
