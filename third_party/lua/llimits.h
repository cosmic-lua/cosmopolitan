...
#else
// [jart] soft assertions stay active outside LUAI_ASSERT builds, so code
//        that only exists to feed one (guarded with assert_code) must run
//        too, or the assert's own expression stops compiling.
